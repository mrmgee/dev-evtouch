...
class PowerSliderLiteBlockController extends BlockController {
	
	var $pobj;
	
	protected $btTable = 'btPowerSlide';
	...
	function duplicate($nbID) {
		...
		foreach($this->images as $im) {
			$db->Execute('insert into btPowerSlideImg (bID, fID, pageID, powerSlidePhraseTitle, powerSlidePhraseDesc, groupSet, position) values (?, ?, ?, ?, ?, ?, ?)', 
				array($nbID, $im['fID'], $im['pageID'],$im['powerSlidePhraseTitle'],$im['powerSlidePhraseDesc'], $im['groupSet'], $im['position'])
			);		
		}
	}
		...
		if( $data['type'] == 'FILESET' && $data['fsID'] > 0){
			...
			foreach($data['imgFIDs'] as $imgFID){ 
				...
				$vals = array(intval($this->bID),intval($imgFID), intval($data['pageID'][$pos]),
						trim($data['powerSlidePhraseTitle'][$pos]),trim($data['powerSlidePhraseDesc'][$pos]),
						intval($data['groupSet'][$pos]),$pos);
				$db->query("INSERT INTO btPowerSlideImg (bID,fID,pageID,powerSlidePhraseTitle,powerSlidePhraseDesc,groupSet,position) values (?,?,?,?,?,?,?)",$vals);
				$pos++;
			}
		}

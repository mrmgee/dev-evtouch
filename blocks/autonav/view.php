...
*/
$pageName = $page->getCollectionHandle(); /* works shows page title */
/* $pageName = $c->cParentID; works shows parent page ID */

$parent = Page::getByID($c->getCollectionParentID());
$grandParent = Page::getByID($parent->getCollectionParentID());  /* Restoration Projects parent, need grandparent  */ 
/* $pageName = $grandParent->getCollectionName(); */  /* Change works  */ 
/* $pageName = $grandParent->getCollectionHandle(); */

/* echo '<div class="nHome '.$pageName.'"><a href="/'.$pageName.'"><span>pageName: '.$pageName.'</span></a></div>'; */

echo PHP_EOL.'<ul class="nav">'.PHP_EOL; //opens the top-level menu

$pageNum = 1;

foreach ($navItems as $ni) {
//TEST
//echo '<h2>url:'.$ni->url.' | name:'.$ni->name.'</h2>';
	
	if ($ni->isEnabled) {
/*	echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">'.$ni->name.' pageNum: '.$pageNum.'</a>';
	*/
	
		if ($ni->name == "Stats") {
			echo '<li id="'.$ni->url.'" class="iStats">'.PHP_EOL;
		} else if ($ni->name == "Plant/Animal ID") {
			echo '<li id="'.$ni->url.'" class="ipaID">'.PHP_EOL;
		} else {
			$iconClass = empty($ni->classes) ? 'icon'.$pageNum : $ni->classes;
			echo '<li id="'.$ni->url.'" class="'.$iconClass.'"><h3>'.$ni->name.'</h3>'.PHP_EOL; //opens a nav item
		}
...
echo '<div class="clear"></div></ul>'; //closes the top-level menu
...
class VideoOverlayBlockController extends BlockController {
	
	protected $btName = 'Video overlay';
	protected $btDescription = 'Video poster frame with player and content in overlay';
	protected $btTable = 'btDCVideoOverlay';
...
//		$this->set('field_5_image', (empty($this->field_5_image_fID) ? null : File::getByID($this->field_5_file_fID)));	//DC file option
		$this->set('field_5_image', $this->get_video_object($this->field_5_image_fID));
	...
	//Helper function for video fields
	private function get_video_object($fID) {
	//	Loader::model('file');
		if (empty($fID)) {
			$video = null;
		} else {
			...
			$fileVer = $file->getFile();
			
//			$video->$passID = $fID;
			
			$video->ftype = $file->getType();
	//		$video->$url = $fileVer->getURL();	//Returns error
	//		$video->$url = $fileVer->getVersion()->getUrl();	//Returns error
			$video->fpath = $file->getPath();
			
			$video->fname = $file->getFileName();
			$video->ftitle = $file->getTitle();
	
	//			$file = $controller->getFileObject();
	//			$rel_file_path=$file->getRelativePath(); 
			
			$video->fp = File::getRelativePathFromID($fID);
			
			$video->fpath = $file->getPath();
			
			$video->fdown = $file->getDownloadUrl();
	
			
	//		$video->src = $file->getRelativePath();	//blank
			$video->src = $file->getVersion()->getRelativePath();
...
(
    [ftype] => Quicktime
    [fpath] => /Applications/MAMP/htdocs/files/6513/3943/3805/ecocenter_tides.mov
    [fname] => ecocenter_tides.mov
    [ftitle] => ecocenter_tides.mov
    [fp] => /files/6513/3943/3805/ecocenter_tides.mov
    [fdown] => http://localhost:8888/index.php/download_file/45/156/
    [src] => /files/6513/3943/3805/ecocenter_tides.mov
    [width] => 
    [height] => 
)
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('VideoOverlayBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('VideoOverlayBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('VideoOverlayBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('VideoOverlayBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('VideoOverlayBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('VideoOverlayBlockController', 'replaceDownloadFileID'),
			$text);

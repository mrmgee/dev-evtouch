...
class MatchingBlockController extends BlockController {
	
	protected $btName = 'Matching';
	protected $btDescription = '';
	protected $btTable = 'btDCMatching';
	...
	public function getSearchableContent() {
		...
		$content[] = $this->field_7_wysiwyg_content;
	...
	public function view() {
		...
		$this->set('field_7_wysiwyg_content', $this->translateFrom($this->field_7_wysiwyg_content));
	...
	public function edit() {
		...
		$this->set('field_7_wysiwyg_content', $this->translateFromEditMode($this->field_7_wysiwyg_content));
	...
	public function save($args) {
		...
		$args['field_7_wysiwyg_content'] = $this->translateTo($args['field_7_wysiwyg_content']);
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('MatchingBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('MatchingBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('MatchingBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('MatchingBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('MatchingBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('MatchingBlockController', 'replaceDownloadFileID'),
			$text);

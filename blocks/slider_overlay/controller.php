...
class SliderOverlayBlockController extends BlockController {
	
	protected $btName = 'Slider overlay';
	protected $btDescription = 'Slider overlay detail content';
	protected $btTable = 'btDCZoomInfo';
	...
	public function getSearchableContent() {
		...
		$content[] = $this->field_5_wysiwyg_content;
	...
	public function edit() {
		$this->set('field_5_wysiwyg_content', $this->translateFromEditMode($this->field_5_wysiwyg_content));
	}

	public function save($args) {
		$args['field_5_wysiwyg_content'] = $this->translateTo($args['field_5_wysiwyg_content']);
		parent::save($args);
	}
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('SliderOverlayBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('SliderOverlayBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('SliderOverlayBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('SliderOverlayBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('SliderOverlayBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('SliderOverlayBlockController', 'replaceDownloadFileID'),
			$text);

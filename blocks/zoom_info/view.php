...
if ($isLogin == 1) {
	...
	echo '<h3>'.$curBl.' '.$field_6_textbox_text.'</h3>';
} else {
	echo '<div id="'.$curBl.'" class="zoomInfoBtn '.$field_6_textbox_text.'" style="'.$zoomStyles.'">';
}
echo '</div>'.PHP_EOL;

if ($isLogin == 1) {
	echo '<div><h3>Info '.$curBl.'</h3>'.PHP_EOL;  // EDIT don't hide div
} else {
	echo '<div class="hideCont">'.PHP_EOL;  // START hider div
}
echo '	<div id="item'.$curBl.'Info" class="zoomInfoCont">'.PHP_EOL;
//echo '	<div class="ctrCont"></div>'.PHP_EOL;
if (!empty($field_5_wysiwyg_content)) {
	echo $field_5_wysiwyg_content;
}
echo PHP_EOL.'<div class="clear"></div></div><!-- END zoomInfoCont -->'.PHP_EOL;
echo '</div><!-- END hideCont -->';
?>
	...
	*/
	...
	$.subscribe(container_name + ".mediator.scopeChange", function() {
		...
		if (focus > tbounds.last) {
			...
			me.dragScopeState = ["over-right", $(TICKS).position().left+227]; //Orig 227px one tick unit

		} else if (focus <= tbounds.first) {
			// over left end of timeline(s): stop rightward drag
			me.dragScopeState = ["over-left", $(TICKS).position().left-227]; //Orig 227px one tick unit
		} else {

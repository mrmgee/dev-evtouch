...
tg.TG_PlayerView = function (widget, mediator) {
	...
	this._views = {
    		...
    		TICKS : PL + " .timeglider-ticks",		//Orig
    		HANDLE : PL + " .timeglider-handle",	//Orig
    		
//    		TICKS : PL + " .timeglider-handle",		//MG
//    		HANDLE : PL + " .timeglider-ticks",	//MG
	...
	*/
	...
	this._templates = {
		...
		event_modal_small: "<div class='tg-modal timeglider-ev-modal ui-widget-content' id='${id}_modal'>"	//MG this creates modal html
...
//		+ "<div class='dateline'>{{html dateline}}</div>"	//Orig
		+ "<div class='dateline "+appCat+"'><h2>${title}</h2></div>"	//MG swap year and title
//		+ "<table><tr><td>"	//Orig
//		+ "<div class='modImg'>{{html image}}</div>"	// img left
		+ "<div class='cont'>"	//Wrap div in float 800w
		+ "{{html image}}"	// img left
//		+ "<h4>${title}</h4>"	//Orig
		+ "<div class='description'>"	// date text right
		+ "<div class='"+appCat+"C'>{{html dateline}}</div>"	//MG swap year and title
		+ "<p>{{html description}}</p>"
		+ "</div>"
		+ "<div class='clear'></div></div>"	//END Wrap div in float 800w
//		+ "</td><td>"	//Orig

//		+ "<div id='insert'></div>"	//Orig
//		+ "</td></tr></table>"	//Orig
//		+ "<div class='footer'><ul>{{html links}}</ul></div>"	//Orig
		+ "</div>"),
...
/*	
		.delegate(".close-button-remove", "click", function () {	//Orig
			var parent_id = $(this).parent().attr("id");
			$("#" + parent_id).remove();		
	})
*/	
/*	
		.delegate(".close-button-remove", "mousedown", function () {	//MG modal close
			var parent_id = $(this).parent().attr("id");
			$("#" + parent_id).remove();
alert('md close parent_id:'+parent_id);		//MG Testing	
	})
*/	
	//MG disabled so only close btn works
	
//		.delegate(".full_modal_scrim, .full_modal_close", "click", function () {	//Orig overlay mask
//		.delegate(".full_modal_close", "mousedown", function () {	//MG works trying alt
		.delegate(".full_modal_close", "mouseup", function () {
			$(".full_modal").remove();
	})
	...
	$(TICKS)
  	.draggable({ axis: 'x',
		...
		drag: function(event, ui) {
			...
			MED.setTicksOffset(t1Left); //Orig
...
/*
			if(t1Left < 3500) {
				...
				t1Left = 3500;
				MED.setTicksOffset(t1Left);
			//	return false;	//Close but not able to drag again once it hits left limit
			}
*/			
			
/*			
			} else {
alert('Before 1970');  //Alert shows
			}
*/
...
*/			
			...
			t2Left = t1Left;
//MG TESTING
//alert('t1Left:'+t1Left+' | ticksSpeed:'+ticksSpeed);			
//			var dsState = me.dragScopeState	//Orig
			...
			if (dsState[0] == "over-left") {
				...
				$(TICKS).css("left", dsState[1]-5);
					return false;
//alert('dsState: '+dsState); //MG testing		dsState: over-left,4103

			} else if (dsState[0] == "over-right") {
				
				$(TICKS).css("left", dsState[1]+5);
					return false;
//alert('dsState: '+dsState); //MG testing	dsState: over-right,-5528
			} else {
				return true;
			}		
		},
...
*/	//MG END remove delegate mouseover
	
	
//MG remove delegate mouseout
	/*
	.delegate(".timeglider-timeline-event", "mouseout", function () { 

		var ev = MED.eventCollection.get($(this).attr("id")).attributes;
//MG remove eventUnHover	
...
//MG added mousedown
//	/*
	.delegate(".timeglider-timeline-event", "mousedown", function () {
//		var ev = MED.eventCollection.get($(this).attr("id")).attributes;
		var eid = $(this).attr("id"); 
//		var ev = MED.eventCollection.get(eid).attributes;
...
*/	//MG END remove delegate hover
	...
	*/
	...
	//	if (focus > tbounds.last) { //Orig
		if (focus > tbounds.last) {	//228px = 1 year
			// past right end of timeline(s): stop leftward drag
		//	me.dragScopeState = ["over-right", $(TICKS).position().left]; //Orig
		//	me.dragScopeState = ["over-right", -4986]; //Right past 2012 (left:-4986px) snaps back to center marker right at Dec 2011
			me.dragScopeState = ["over-right", -4992]; //Right past 2012 (left:-4986px) snaps back to center marker right at 2012
//alert('RIGHT focus:'+focus+' | tbounds.last: '+(tbounds.last)+' | dragScopeState:'+me.dragScopeState); //MG testing		RIGHT over-right,-4986

//left: -4985px; 


//RIGHT focus:63465328692 | tbounds.last: 63461062740 | dragScopeState:over-right,-4990
//RIGHT focus:63474914525 | tbounds.last: 63461062740 | dragScopeState:over-right,-4992	Jan 2012


//RIGHT focus:63476303776 | tbounds.last: 63461062740 | dragScopeState:over-right,-5100
//RIGHT focus:63464356217 | tbounds.last: 63461062740 | dragScopeState:over-right,-4986


//RIGHT tbounds.last: 63461062740 | dragScopeState:over-right,-4986
//RIGHT focus:63461577714 | tbounds.last: 63461062740 | dragScopeState:over-right,-4986
//+288				RIGHT focus:63490613062 | tbounds.last: 63461062740 | dragScopeState:over-right,-4986
//0 				RIGHT focus:63465050842 | tbounds.last: 63461062740 | dragScopeState:over-right,-4986
//focus+10000		RIGHT focus:63484361433 | tbounds.last: 63461062740 | dragScopeState:over-right,-4986

		} else if (focus < tbounds.first) {
			// over left end of timeline(s): stop rightward drag
		//	me.dragScopeState = ["over-left", $(TICKS).position().left]; //Orig
			me.dragScopeState = ["over-left", 3444]; //Left past 1972 (left:4099px) snaps back to Left edge right at 1971		
		} else {
...
//		$(DATE).text(fd.format("d MMM yyyy", false));	//Orig
		$(DATE).text(fd.format("MMM yyyy", false));	//Only show year
	},
	...
	*/
	registerEventImages : function ($timeline) {
      ...
      $(CONTAINER + " .timeglider-event-image-above").each(
    		    function () {
//    		    var numRandSrc = Math.floor(Math.random()*101); //MG random number between 1-100
//    		    var numRand = '-'+((numRandSrc*2)+600);	//MG makes random difference larger mult by 2 add 600
...
//	           				at: "'+numRand+'",	//MG
            				of: $(CONTAINER),
            				offset: "0, 12"		//MG  Pixel values for offset, Horizontal then vertical, negative values OK
    	        }).css({left:0});
    	        
    	        $(this).css("display", "block").css({top:'+topSrc+'}).css({left:0}); //Orig
//    	        $(this).css("display", "block").css({left:0}).css('top', (numRand) + 'px');	//MG randomizes top of above image
...
//			ev_img = (ev.image && ev.image.src) ? "<img src='" + ev.image.src + "'>" : "",	//Orig
			ev_img = (ev.image && ev.image.src) ? "<img src='" + ev.image.src + "' ondragstart='return false'>" : "",	//MG added to prevent drag large imgs overlay
...
//	  	    			"top":((ch - ph)/2) + "px",	//Orig
 ...
 //     				.draggable()  //MG remove drag from modal
//      				.hover(function () { $(this).css("z-index", me.ztop++); });	//MG remove hover form modal

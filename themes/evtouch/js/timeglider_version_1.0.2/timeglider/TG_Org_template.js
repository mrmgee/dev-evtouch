						...
						if (b.html && b.html.substr(0,4) == "<div") {
							...
							if (polarity_cond){
								...
								if (b.fontsize > 2) {
									...
									var testTemplate = "<div id='<%=id%>' class='<%=_css%>' style='width:<%=width%>px; height:<%=height%>px; left:<%=left%>px; opacity:<%=opacity%>; top:<%=_top%>px; font-size:<%=fontsize%>px;'><%=_icon%><%=_img%><%=_span_div%><div class='timeglider-event-title' style='top:<%=_title_adj%>px'><%=title%></div></div>";
									...
									b._css = "timeglider-timeline-event " + span_selector_class + lane_class;
									
									html += _.template(testTemplate, b);

...
//		+ "<h4>${title}</h4><div class='tg-full_modal-vidimg'></div>"
		...
		+ "<div class='tg-date "+appCat+"C'>{{html dateline}}</div>"	//MG swap year and title
		+ "<p>{{html description}}</p>"
//		+ "</div>" //close cont
		+ "<div class='clear'></div></div>"	//END Wrap div in float 800w
		+ "</div>" //close tg-full_modal-body
      	...
      	if (this.passesFilters(ev, zl) === true) {
      		...
      		if (expCol == "expanded") {
				...
				ev.height = ev.fixed_height || (font_ht + 4);
      			...
      			// debug.log("fixing at 60", ev.height);
...
//      					"width":pw,
//      					"height":ph,
      	    			"top":((ch - ph)/3) + "px",	//MG 21
    			  		"left":((cw - pw)/2) + "px"	//MG 32
    	  			});
...
//   	  			if ($pp.height() > pph-16) {	//MG height limit and scroll not needed
